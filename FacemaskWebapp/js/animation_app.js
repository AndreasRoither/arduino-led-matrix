...
/* This function is stolen from: https://stackoverflow.com/questions/5623838/rgb-to-hex-and-hex-to-rgb
Written by Tim Down */
...
class Animation {
  ...
  invert() {
    let oldLeds = this.leds;
    let newArr2d = [];
    let newArr1d = [];

    // convert to 2d array
    while (this.leds.length)
      newArr2d.push(this.leds.splice(0, CONFIG["grid_length"]));

    /*for(var i = newArr2d.length - 1; i > 0; i--) {
      newArr2d2.push(newArr2d[i])
    }*/

    // swap rows
    for (var i = 0; i < newArr2d.length / 2; i++) {
      for (var j = 0; j < CONFIG["grid_length"]; j++) {
        var temp = newArr2d[i][j].colorState.slice(0);

        newArr2d[i][j].colorState =
          newArr2d[newArr2d.length - 1 - i][j].colorState;
        newArr2d[newArr2d.length - 1 - i][j].colorState = temp;
      }
    }

    // convert to 1d array
    for (var i = 0; i < newArr2d.length - 1; i++) {
      this.leds = this.leds.concat(newArr2d[i]);
    }
  }
  ...
  export() {
    ...
    let invertCheck = document.getElementById("invertedCheckbox").checked;
  ...
  stepBackward() {
    this.currentIndex--;
    if (this.currentIndex < 0) this.currentIndex = this.stepCount;
    this.update();
  }
  ...
  draw() {
    ...
    CTX.beginPath();
    var x =
      (CONFIG["led_size"] * CONFIG["grid_length"] + CONFIG["grid_length"] * CONFIG["led_spacing"] - 1) / 2 - CONFIG["led_spacing"] / 2;
    var height =
      CONFIG["led_size"] * CONFIG["grid_height"] + CONFIG["grid_height"] * CONFIG["led_spacing"];
    var y =
      (CONFIG["led_size"] * CONFIG["grid_height"] + CONFIG["grid_height"] * CONFIG["led_spacing"] - 1) / 2 - CONFIG["led_spacing"] / 2;
    var length =
      CONFIG["led_size"] * CONFIG["grid_length"] + CONFIG["grid_length"] * CONFIG["led_spacing"];

    CTX.fillStyle = "#FFFFFF";
    // /4 since we have left and right some space
    CTX.fillRect(x, 0, CONFIG["led_spacing"] / 4 , height);
    CTX.fillRect(0, y, length, CONFIG["led_spacing"] / 4);
    CTX.stroke();
...
class Led {
  ...
  draw() {
    if (
      this.colorState[this.currentState - 1] != CONFIG["none_color"] &&
      this.colorState[this.currentState] == CONFIG["none_color"] &&
      this.currentState != 0 &&
      !playing
    ) {
    ...
    CTX.beginPath();
    CTX.fillRect(
      this.x * this.size + this.x * this.spacing,
      this.y * this.size + this.y * this.spacing,
      this.size,
      this.size
    );
    CTX.stroke();
...
function startDraw() {
  ...
  // to stop annoying continuous drawing even when you left the canvas
